              ...
              <div className="p-6 space-y-2">
                ...
                {!token ? (
                  <div className="grid grid-cols-2 gap-4">
                    <Link to="/login" className="flex items-center justify-center py-4 bg-slate-50 text-slate-600 rounded-2xl font-bold" onClick={() => setIsMenuOpen(false)}>Login</Link>
                    <Link to="/signup" className="flex items-center justify-center py-4 bg-blue-600 text-white rounded-2xl font-bold shadow-lg shadow-blue-200" onClick={() => setIsMenuOpen(false)}>Join</Link>
                  </div>
                ) : (
                  <div className="space-y-2">
                    <Link to="/profile" className={linkClass("/profile")} onClick={() => setIsMenuOpen(false)}>Profile</Link>
                    <button onClick={logout} className="w-full py-4 text-red-500 font-bold border border-red-50 border-t-0 rounded-2xl mt-4 bg-red-50/50">
                      Sign Out
                    </button>
                  </div>
                )}
